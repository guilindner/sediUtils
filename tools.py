...
def generateAtoms(atoms, domainBox, safeBox):
    ...
    with open('In_initialPython3', 'w') as myfile:
...
def freezeAtoms(inputFile,domainBox):
    ...
    with open('In_Frozen', 'w') as fileout:
...
def activateAtoms(inputFile,atomType,box):
    ...
    temp = []
    with open('In_Active.in', 'w') as myfile:
        for i in range(0,10):
...
def removeAtoms(inputFile, atoms, box):
    ...
    with open('In_Removed.in', 'w') as myfile:
        for i in range(10):
            if i == 2:
                myfile.write(str(len(temp_data))+" atoms\n")
            elif i == 3:
...
def ofToLammps(inputFile, atoms, domainBox, box):
    """Keep OF particles inside the box
    
    Keep particles inside the box, while removing any atom outside the box.
    ...
    for i in range(19,finalLine):
        ...
        if (float(modLine[0]) < float(box[1])) and (float(modLine[1]) < float(box[3])) and (float(modLine[2]) < float(box[5])):
            temp_data.append([modLine[0],modLine[1],modLine[2],modLine[3],1])

    with open('In_OF.in', 'w') as myfile:
        myfile.write("sphere data\n\n")
        myfile.write(str(len(temp_data))+" atoms\n")
        myfile.write(str(atoms['nType'])+" atom types\n\n")
        myfile.write(" "+str(domainBox[0])+" "+str(domainBox[1])+" xlo xhi\n")
